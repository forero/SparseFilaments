module ap_tools
	...
	implicit none
	integer, parameter :: char_len = 500
...
!		integer, allocatable :: nbins_list(:) ! abondoned !
	...
	! useful variabls: used for common test
	character(len=char_len) :: teststr, teststr1, teststr2, teststr3, teststr4, teststr5, teststr6, teststr7, & 
		testfile, testfile1, testfile2, testfile3, testfile4, testfile5, testfile6, testfile7
	real(dl) :: testx, testy, testz, testx1, testx2, testx3, testx4, testx5, testx6, testx7, &
		tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp7
	real(dl), allocatable :: tmpAY1(:), tmpAY2(:)
...
33		write(*,*) "Error occurs when writing into the file ", file_name
  ...
  !------------------------------------------
	...
	! the same as before, but with 'adapted' precision
  ...
  !---------------------------------------------------------------
  	subroutine eqvl_binned_quan(quan_val_list, quan_r_list, rmin, rmax, ndat, num_bins, &
  		quan_val_av_list, quan_val_er_list, r_av_list, quan_val_var_list, quan_medval_list, numinbinlist, redgelist)
  		! Dummy
  		real(dl), intent(in) :: quan_val_list(ndat), quan_r_list(ndat) 
  		real(dl), intent(in) :: rmin, rmax
  		integer, intent(in) :: ndat, num_bins
  		real(dl), intent(out) :: quan_val_av_list(num_bins), quan_val_er_list(num_bins), r_av_list(num_bins)
  		real(dl), optional, intent(out) :: quan_val_var_list(num_bins), quan_medval_list(num_bins), numinbinlist(num_bins), &
  			redgelist(num_bins+1)
  		! Local
  		...
  		if(present(redgelist)) then
  			redgelist(1) = rmin
  			do i = 1, num_bins
  				redgelist(i+1) = (redgelist(i)**3.0 + deltavol)**(1.0/3.0)
  			enddo
  		endif
		...
		! which elements in which bin
  		do binned_i = 1, num_bins
	  		m = binned_num_list(binned_i) 
  			binned_quan_val_list(binned_i)%n = m
  			if(present(numinbinlist)) numinbinlist(binned_i) = m
  			binned_r_list(binned_i)%n = m
  			allocate(binned_quan_val_list(binned_i)%array(m))
  			allocate(binned_r_list(binned_i)%array(m))
  		enddo
