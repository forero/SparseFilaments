...
implicit none
	...
	real(dl), public :: gb_amp_changenuminx = 0.04_dl
...
!			chisqlist(idata) = chisq_of_mu_data2(mudata)	
  ...
  ! calculating chisq
  !------------------------------------------
	  subroutine gf_mldprho_mlchi2s(omegam, w, h, cs, rho_chisqlist, rho_dfchisqlist, &
	  	dlt_chisqlist, dlt_dfchisqlist, ndlt_chisqlist, ndlt_dfchisqlist, numchisq, calc_comvr, gvfastmode)
		! dummy
		...
		real(dl), intent(out) :: rho_chisqlist(numchisq), rho_dfchisqlist(numchisq), &
			dlt_chisqlist(numchisq), dlt_dfchisqlist(numchisq), &
			ndlt_chisqlist(numchisq), ndlt_dfchisqlist(numchisq)
		logical, intent(in) :: calc_comvr 
		logical, optional, intent(in) :: gvfastmode
		! local
		integer :: i, j, RSD, AP
		real(dl), allocatable :: changenuminxlist(:)
		real(dl) :: tmprhochisqlist(numchisq), tmprhodfchisqlist(numchisq), &
			tmpdltchisqlist(numchisq), tmpdltdfchisqlist(numchisq), &
			tmpndltchisqlist(numchisq), tmpndltdfchisqlist(numchisq)
		logical :: fastmode
		
		if(present(gvfastmode)) then
			fastmode = gvfastmode
		else
			fastmode = .false.
		endif
...
!		cs%print_info = .true.
		...
		!get the chisqs
		allocate(changenuminxlist(2*gb_num_changenuminx+1))
...
!			endif
		...
		rho_dfchisqlist = 0.0_dl
		dlt_chisqlist = 0.0_dl
		dlt_dfchisqlist = 0.0_dl
		ndlt_chisqlist = 0.0_dl
		ndlt_dfchisqlist = 0.0_dl
		do i = 1, size(changenuminxlist)
!			print *, 'Calculating many chisqs (with pixels rescaling): i = ', i
			call gd_mldprho_mlchi2s(RSD, AP, cs, changenuminxlist(i), tmprhochisqlist, tmprhodfchisqlist, &
				tmpdltchisqlist, tmpdltdfchisqlist, tmpndltchisqlist, tmpndltdfchisqlist, cs%numdrop, fastmode)
!			print *, '(gf_mldprho_mlchi2s) Checking tmplist: ', tmprhochisqlist, tmprhodfchisqlist
			if(gbtp) then
!				write(*,'(A,f9.4,A,<cs%numdrop>(f13.7,1x))'), '   chisqs at changenuminx = ', real(changenuminxlist(i)), ':', &
!					 real(tmpchisqlist(1:cs%numdrop))
			endif
			do j = 1, cs%numdrop
				rho_chisqlist(j) = rho_chisqlist(j) + tmprhochisqlist(j) / (size(changenuminxlist) + 0.0)
				dlt_chisqlist(j) = dlt_chisqlist(j) + tmpdltchisqlist(j) / (size(changenuminxlist) + 0.0)
				ndlt_chisqlist(j) = ndlt_chisqlist(j) + tmpndltchisqlist(j) / (size(changenuminxlist) + 0.0)
				rho_dfchisqlist(j) = rho_dfchisqlist(j) + tmprhodfchisqlist(j) / (size(changenuminxlist) + 0.0)
				dlt_dfchisqlist(j) = dlt_dfchisqlist(j) + tmpdltdfchisqlist(j) / (size(changenuminxlist) + 0.0)
				ndlt_dfchisqlist(j) = ndlt_dfchisqlist(j) + tmpndltdfchisqlist(j) / (size(changenuminxlist) + 0.0)
			enddo
		enddo
		deallocate(changenuminxlist)
!		print *, '(gf_mldprho_mlchi2s) Checking rholist: ', rho_chisqlist, rho_dfchisqlist
	end subroutine gf_mldprho_mlchi2s
