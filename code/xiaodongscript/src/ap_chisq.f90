...
!			chisqlist(idata) = chisq_of_mu_data2(mudata)	
  ...
  ! calculating chisq based
  !------------------------------------------
	  subroutine gf_mldprho_chi2s(omegam, w, h, cs, rho_chisqlist, calc_comvr)
		! dummy
		real(dl), intent(in) :: omegam, w, h
		type(chisq_settings), intent(inout) :: cs
		real(dl), intent(out) :: rho_chisqlist(:)
...
!		cs%print_info = .true.
		...
		!get the chisqs
		allocate(changenuminxlist(2*gb_num_changenuminx+1), tmpchisqlist(cs%numdrop))
