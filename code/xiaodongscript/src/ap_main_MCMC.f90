...
!		stop
	...
	read(1,*) gb_chisq_method, gb_num_changenuminx, gb_amp_changenuminx
	...
	write(*,'(A,i4)') '     method of chisq   : ', gb_chisq_method
	if(gb_chisq_method .eq. 2) then
		tmpint = num_nbins
		write(*,*) '    num_nbins         = ', tmpint
		write(*,'(A,<num_nbins>(i4,2x))') '     list of nbins     : ', cs%nbins_list	
	endif
	
	if(gb_chisq_method .eq. 3) then
