...
contains
  ...
  !------------------------------------------
	subroutine init_pixels(RSD, AP, rl_num_in_x, print_info, use_num_density, do_xyzmassinit)
		...
		call do_cell_init(RSD, AP, rl_num_in_x, gv_print_info = .false., do_not_init_xyz_mass = .false., &
			do_not_init_cellmat = .true.)
...
!		call find_min_max(r_list,size(r_list),rmin,rmax)
		...
		delta_r = (boundary_rmax - boundary_rmin) / (nbins_rhoav + 0.0)
  		...
  		do i_sm = 1, nsm
  			call binned_quan(quan_list, distance_list, boundary_rmin, boundary_rmax, nbins_rhoav, &
